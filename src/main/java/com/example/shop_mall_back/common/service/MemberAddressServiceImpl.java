...
public class MemberAddressServiceImpl implements MemberAddressService{
    ...
    public Long addMemberAddress(MemberAddressDTO memberAddressDTO, Long id) {
        ...
        // 기본 주소지 추가
        if (memberAddressDTO.isDefault() || memberAddressDTO.getId() == 1){
            memberAddressRepository.resetDefaultAddressByMemberId(member.getId());
        }

        // 유저 address 객체생성
        MemberAddress memberAddress = MemberAddress.addAddress(member, memberAddressDTO.getZipcode(), memberAddressDTO.getAddress(), memberAddressDTO.getAddressDetail(), memberAddressDTO.isDefault(), memberAddressDTO.getNote());
    ...
    public void memberAddressUpdate(MemberAddressDTO memberAddressDTO) {
        ...
        // isDefault 설정
        if(memberAddressDTO.isDefault()){
            memberAddressRepository.resetDefaultAddressByMemberId(memberAddressDTO.getId());
        }
